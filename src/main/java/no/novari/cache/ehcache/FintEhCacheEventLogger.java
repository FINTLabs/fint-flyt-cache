...
public class FintEhCacheEventLogger<K, V> extends FintEhCacheEventListener<K, V> {
    ...
    public void onEvent(FintCacheEvent<K, V> event) {
        switch (event.getType()) {
            case CREATED -> log.info("Cache entry in '{}' with key='{}' created with value={}", this.cacheAlias, event.getKey(), event.getNewValue());
            case UPDATED -> log.info("Cache entry in '{}' with key='{}' updated from {} to {}", this.cacheAlias, event.getKey(), event.getOldValue(), event.getNewValue());
